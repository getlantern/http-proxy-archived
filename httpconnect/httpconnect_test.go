...
func TestFilterTunnelPorts(t *testing.T) {
	...
	server := httptest.NewServer(filters.Join(
		New(&Options{AllowedPorts: []int{port}}),
	...
	req, err := http.NewRequest("GET", "https://site.com", nil)
	resp, _ := client.Do(req)
	assert.Nil(t, resp, "CONNECT request without port should fail with 400")

	req, _ = http.NewRequest("GET", "https://site.com:", nil)
	resp, _ = client.Do(req)
	assert.Nil(t, resp, "CONNECT request without port should fail with 400")
